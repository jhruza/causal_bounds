...
# generate graph_full
graph_full <- initialize_graph(igraph::graph_from_literal(Z -+ X, S -+Y , X- + Y, 
                                    Ur -+ S, Ur -+ X, Ur -+ Y))
# print(V(graph))
plot(graph_full)
...
for (param in obj_full$parameters){
    ...
    joint_p<-sum(subset(p$X, outcome == x & Z == z)[ ,"p"]*subset(p$Y, outcome == y &  S == s & X == x)[ ,"p"]*subset(p$S, outcome == s)[ ,"p"]*subset(p$Z, outcome == z)[ ,"p"]*subset(p$Ur)[ ,"p"]) # nolint
