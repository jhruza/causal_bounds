...
# generate graph_full
graph_full <- initialize_graph(igraph::graph_from_literal(Ul -+ Z, Ul-+S, Z -+ X, S-+X, S -+Z , X- + Y, 
                                    Ur -+ X, Ur -+ Y))
# print(V(graph))
plot(graph_full)
# Assuming V(graph_full)$name = ["Ul","Z", "s", "X", "Y", "Ur"]
V(graph_full)$name
V(graph_full)$leftside <- c(1, 1, 1, 0, 0, 0)
V(graph_full)$latent   <- c(1, 0, 0, 0, 0, 1)
V(graph_full)$nvals    <- c(2, 2, 2, 2, 2, 2)
...
for (param in obj_full$parameters){
    ...
    s<- as.numeric(substring(param, 6, 6))

    joint_p<-sum(subset(p$X, outcome == x & Z == z)[ ,"p"]*subset(p$Y, outcome == y & X == x)[ ,"p"]*subset(p$S, outcome == s)[ ,"p"]*subset(p$Z, outcome == z & S==s)[ ,"p"]*subset(p$Ur)[ ,"p"]) # nolint
