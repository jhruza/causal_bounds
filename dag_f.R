...
# non iv
graph_no_iv <- initialize_graph(igraph::graph_from_literal(A-+Y, Ur-+A, B, Ur-+ Y))
plot(graph_no_iv)
V(graph_no_iv)$name
# Assuming V(graph_full)$name = "A"  "Y"  "Ur" "B"
V(graph_no_iv)$name
V(graph_no_iv)$leftside <- c(0, 0, 0, 0)
V(graph_no_iv)$latent   <- c(0, 0, 1, 0)
# V(graph_no_iv)$nvals    <- c(2, 2, 2, 2)
V(graph_no_iv)$nvals    <- c(3, 2, 2, 3)
...
to_bound <- "p{Y(A = 0) = 1; B=0}+p{Y(A = 1) = 1; B=1}"
...
# IV setting
graph_iv <- initialize_graph(igraph::graph_from_literal(Z -+A, A-+Y, Ur-+A, Ur-+ B, Ur-+ Y))
plot(graph_iv)
V(graph_iv)$name
# Assuming V(graph)$name = "Z"  "A"  "Y"  "Ur" "B" 
V(graph_iv)$name
V(graph_iv)$leftside <- c(1, 0, 0, 0, 0)
V(graph_iv)$latent   <- c(0, 0, 0, 1, 0)
# V(graph_iv)$nvals    <- c(2, 2, 2, 2, 2)
V(graph_iv)$nvals    <- c(2, 3, 2, 2, 3)
